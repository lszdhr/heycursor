...
var ROOT_DATA_DIR = path.join(os.homedir(), ".windsurf-mcp-messenger");
...
<title>Windsurf Messenger - \u8FDC\u7A0B\u63A7\u5236\u53F0</title>
<style>
...
<body>
<div class="wrap">
	<div class="hdr"><h1>Windsurf Messenger</h1><p>\u8FDC\u7A0B\u63A7\u5236\u53F0</p></div>
	<div class="stat-row">
	...
	<div class="card highlight">
		...
		<div class="card-body">
			<div class="compose-area">
				<textarea id="msgInput" class="compose-input" placeholder="\u8F93\u5165\u6D88\u606F\u53D1\u9001\u7ED9 Windsurf..." rows="3"></textarea>
...
// src/rules.ts
var MCP_DISPLAY_NAME = "Windsurf Messenger";
var RULES_FILE_NAME = "mcp-messenger.md";
var RULES_CONTENT = `---
...
5. \u9700\u8981\u7528\u6237\u505A\u9009\u62E9\u6216\u786E\u8BA4\u65F6\uFF08\u9009\u65B9\u6848\u3001\u9009\u6846\u67B6\u3001\u786E\u8BA4\u64CD\u4F5C\u7B49\uFF09\uFF0C\u5FC5\u987B\u8C03\u7528 \`ask_question\`\uFF1B\u4E25\u7981\u4F7F\u7528 Windsurf \u5185\u7F6E\u7684 ask_user_question
...
// src/windsurf-usage.ts
function getWindsurfConfigDir() {
  switch (process.platform) {
    case "win32":
      return path3.join(process.env.APPDATA || path3.join(os3.homedir(), "AppData", "Roaming"), "Windsurf");
    case "darwin":
      return path3.join(os3.homedir(), "Library", "Application Support", "Windsurf");
    default:
      return path3.join(os3.homedir(), ".config", "Windsurf");
  }
}
function readWindsurfStateDb(key) {
  const dbPath = path3.join(getWindsurfConfigDir(), "User", "globalStorage", "state.vscdb");
  ...
  try {
    ...
    const script = `const{DatabaseSync}=require("node:sqlite");const db=new DatabaseSync('${escaped}',{readOnly:true});const r=db.prepare("SELECT value FROM ItemTable WHERE key=?").get("${keyEsc}");db.close();console.log(r?.value||"")`;
    return execSync(`node --disable-warning=ExperimentalWarning -e "${script}"`, {
      encoding: "utf-8",
      timeout: 1e4,
      windowsHide: true
    }).trim() || null;
...
async function fetchUsageStub() {
  try {
    const planRaw = readWindsurfStateDb("windsurf.settings.cachedPlanInfo");
    if (!planRaw)
      return { success: false, error: "\u672A\u627E\u5230 Windsurf \u5957\u9910\u7F13\u5B58\u4FE1\u606F" };
    const plan = JSON.parse(planRaw);
    const u = plan.usage || {};
    const used = u.usedMessages ?? 0;
    const total = u.messages ?? 0;
    const usagePct = total > 0 ? Math.round((used / total) * 100) : null;
    return {
      success: true,
      email: "",
      membershipType: plan.planName ?? "-",
      usagePct,
      billingCycleStart: plan.startTimestamp ? new Date(plan.startTimestamp).toISOString() : "",
      billingCycleEnd: plan.endTimestamp ? new Date(plan.endTimestamp).toISOString() : "",
      totalCost: 0,
      eventsCount: used,
      models: [],
      windsurf: {
        remainingMessages: u.remainingMessages ?? 0,
        remainingFlowActions: u.remainingFlowActions ?? 0,
        remainingFlexCredits: u.remainingFlexCredits ?? 0,
        billingStrategy: plan.billingStrategy ?? "",
        dailyRemainingPct: plan.quotaUsage?.dailyRemainingPercent ?? 0,
        weeklyRemainingPct: plan.quotaUsage?.weeklyRemainingPercent ?? 0
      }
    };
  } catch (e) {
    return {
...
// src/extension.ts
var MCP_DISPLAY_NAME2 = "Windsurf Messenger";
var ROOT_DATA_DIR2 = path3.join(os3.homedir(), ".windsurf-mcp-messenger");
var WEBVIEW_STATE_KEY = "messengerWebviewState";
var GLOBAL_WINDSURF_DIR = path3.join(os3.homedir(), ".codeium", "windsurf");
var GLOBAL_MCP_JSON = path3.join(GLOBAL_WINDSURF_DIR, "mcp_config.json");
var GLOBAL_RULES_DIR = null;
...
function hasMcpConfig(workspaceFolder) {
  const mcpJsonPath = path3.join(workspaceFolder, ".windsurf", "mcp.json");
...
function setupMcpConfigGlobal() {
  if (!fs3.existsSync(GLOBAL_WINDSURF_DIR)) {
    fs3.mkdirSync(GLOBAL_WINDSURF_DIR, { recursive: true });
  }
  let config = {};
  if (fs3.existsSync(GLOBAL_MCP_JSON)) {
    try {
...
var GLOBAL_RULES_MARKER_START = "<!-- WINDSURF-MESSENGER-RULES-START -->";
var GLOBAL_RULES_MARKER_END = "<!-- WINDSURF-MESSENGER-RULES-END -->";
var GLOBAL_RULES_FILE = path3.join(os3.homedir(), ".codeium", "windsurf", "memories", "global_rules.md");
function setupWindsurfRulesGlobal() {
  const dir = path3.dirname(GLOBAL_RULES_FILE);
  if (!fs3.existsSync(dir))
    fs3.mkdirSync(dir, { recursive: true });
  let existing = "";
  if (fs3.existsSync(GLOBAL_RULES_FILE))
    existing = fs3.readFileSync(GLOBAL_RULES_FILE, "utf-8");
  if (existing.includes(GLOBAL_RULES_MARKER_START))
    return false;
  const block = "\n\n" + GLOBAL_RULES_MARKER_START + "\n" + RULES_CONTENT + "\n" + GLOBAL_RULES_MARKER_END + "\n";
  fs3.writeFileSync(GLOBAL_RULES_FILE, existing + block, "utf-8");
  return true;
}
function ensureRulesInWorkspaces(workspaceFolders) {
  let count = 0;
  for (const folder of workspaceFolders) {
    const rulesDir = path3.join(folder.uri.fsPath, ".windsurf", "rules");
...
function ensureMcpConfigInWorkspaces(workspaceFolders) {
  ...
  const windsurfDir = path3.join(firstFolder, ".windsurf");
  if (!fs3.existsSync(windsurfDir))
    fs3.mkdirSync(windsurfDir, { recursive: true });
  const mcpJsonPath = path3.join(windsurfDir, "mcp.json");
  let config = {};
  if (fs3.existsSync(mcpJsonPath)) {
    try {
...
function removeMcpConfigGlobal() {
  ...
  if (fs3.existsSync(GLOBAL_RULES_FILE)) {
    try {
      const content = fs3.readFileSync(GLOBAL_RULES_FILE, "utf-8");
      const startIdx = content.indexOf(GLOBAL_RULES_MARKER_START);
      const endIdx = content.indexOf(GLOBAL_RULES_MARKER_END);
      if (startIdx !== -1 && endIdx !== -1) {
        let removeStart = startIdx;
        while (removeStart > 0 && content[removeStart - 1] === "\n")
          removeStart--;
        let removeEnd = endIdx + GLOBAL_RULES_MARKER_END.length;
        if (content[removeEnd] === "\n")
          removeEnd++;
        const cleaned = content.substring(0, removeStart) + content.substring(removeEnd);
        fs3.writeFileSync(GLOBAL_RULES_FILE, cleaned, "utf-8");
        removed = true;
      }
    } catch {
    }
...
async function triggerWindsurfChat() {
  if (chatTriggered)
    return;
  chatTriggered = true;
  try {
    ...
    try {
      await vscode.commands.executeCommand("windsurf.openCascade");
    } catch {
    }
  }
}
...
var MessengerViewProvider = class {
  ...
  handlePastedImage(dataUrl, caption) {
    try {
      ...
      const name = "windsurf_mcp_" + Date.now() + "_" + Math.random().toString(36).slice(2, 8) + "." + ext;
  ...
  async handleFetchUsage() {
    ...
    try {
      const result = await fetchUsageStub();
...
function activate(context) {
  ...
  context.subscriptions.push(
    vscode.window.registerWebviewViewProvider(
      "windsurfMcpMessenger.mainView",
      provider
    )
  );
  context.subscriptions.push(
    vscode.commands.registerCommand("windsurfMcpMessenger.setupMcp", () => {
      ...
      if (folders2.length > 0) {
        ...
        setDataDir(ROOT_DATA_DIR2);
        changed = setupMcpConfigGlobal() || setupWindsurfRulesGlobal();
      }
      if (changed) {
        reloadWindowAfterInstall();
      } else {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand("windsurfMcpMessenger.removeMcp", () => {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand("windsurfMcpMessenger.openConsole", () => {
  ...
  startLocalServer(
    ...
    console.log(`Windsurf Messenger \u63A7\u5236\u53F0\u5DF2\u542F\u52A8: http://127.0.0.1:${port}`);
  }).catch((e) => {
